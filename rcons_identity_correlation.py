...
def run(alignment_dir='.', occ=0.10,
    ...
    for (name, ftype, regime, fname,
        ...
        rows.append({
            ...
            'mpsi':     round(mpsi, 4) if mpsi is not None else '',
            'mpsi_std': round(mpsi_std, 4) if mpsi_std is not None else '',
            'n_seq':    n_seq if n_seq is not None else '',
            'L_eff':    L_eff if L_eff is not None else '',
            'source':   source,
        })
